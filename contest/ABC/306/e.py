...
def swap(f_a, rem):
    res = 0
    # f_aの最小値とremの最大値を入れ替える
    while len(rem) > 0 and f_a[0] < rem[-1]:
        to_rem = f_a[0]
        to_f_a = rem[-1]
        f_a.remove(f_a[0])
        rem.remove(rem[-1])
...
def erase(f_a, rem, a_x):
    res = 0
    # f_aにa[x]がある場合、resを調整
    if a[x] in f_a:
        f_a.remove(a_x)
        res += a[x]
    else:
...
def fill_f_a(f_a, rem):
    res = 0
    # eraseによって変化したf_aをfillする
    while len(f_a) < k:
        t = rem[-1]
        f_a.add(t)
        rem.remove(t)
...
f_a = MultiSet(compress=list(s))  # 大きい方からのK個を格納する
rem = MultiSet(compress=list(s))  # f_a以外を格納
f_a.add(0, k)
rem.add(0, n - k)
...
for x, y in xy:
    x -= 1
    rem.add(y)
    # remにyを入れたのでswapする必要があればする
    ans += swap(f_a, rem)
    ans -= erase(f_a, rem, a[x])
    ans += fill_f_a(f_a, rem)
    print(ans)
    a[x] = y
